...
def _write_csv(tmp_path):
    ...
    with open(p, "w", newline="") as f:
        ...
        w.writerow(HEADER)
        for seed in (0, 1):
            for (m, pl, N, df) in (("pca", "solved", 50, 1.8),
                                    ("cca", "solved", 50, 1.8),
                                    ("cca", "constructed", 50, 1.8)):
                w.writerow(["smoke", 1, m, pl, "", N, df, 1.3, 1.0,
                            seed, 1.5 + 0.1 * seed, "ok", "",
                            df - 0.02 + 0.01 * seed, 0.98, 20, 0.01,
                            5.0, 5.1, "", 0.0])
            w.writerow(["smoke", 3, "cca", "solved", 0.3, 100, 1.8, 1.3,
...
def test_analyze_outputs(tmp_path):
    ...
    assert "pca" in tex and "cca" in tex and "constructed" in tex
